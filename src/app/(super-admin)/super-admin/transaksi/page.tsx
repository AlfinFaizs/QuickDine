"use client";
// src/app/(super-admin)/super-admin/transaksi/page.tsx
// Halaman Khusus Log Transaksi Pesanan Nasional & Export Master Ledger Excel
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-xl sm:text-2xl font-black text-[#131b2e]">
            Log Transaksi &amp; Arus Keuangan Nasional
          </h1>
          <p className="text-xs sm:text-sm text-[#6d7a72] mt-0.5">
            Rekap transaksi pesanan masuk dari seluruh restoran mitra se-Indonesia beserta potongan fee platform.
          </p>
        </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
        <div className="rounded-2xl border border-[#bccac0]/30 bg-white p-4 space-y-1">
          <span className="text-[10px] text-[#6d7a72] font-semibold block">Total GMV Nasional</span>
          <span className="text-lg font-black text-[#131b2e]">{formatRupiah(kpi.totalGmv)}</span>
        </div>

        <div className="rounded-2xl border border-[#006948]/30 bg-emerald-50/40 p-4 space-y-1">
          <span className="text-[10px] text-[#006948] font-bold block">Profit Fee Platform</span>
          <span className="text-lg font-black text-[#006948]">{formatRupiah(kpi.totalFeeRevenue)}</span>
        </div>

        <div className="rounded-2xl border border-[#bccac0]/30 bg-white p-4 space-y-1">
          <span className="text-[10px] text-[#6d7a72] font-semibold block">Total Transaksi Hari Ini</span>
          <span className="text-lg font-black text-[#131b2e]">{kpi.totalOrdersToday.toLocaleString("id-ID")} Pesanan</span>
        </div>
      </div>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <h2 className="text-base font-extrabold text-[#131b2e]">
              Rincian Transaksi Masuk Real-Time
            </h2>
            <p className="text-xs text-[#6d7a72]">
              Laporan transaksi per order dengan status settlement bank H+1.
            </p>
          </div>
